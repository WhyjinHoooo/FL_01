...
<div class="ComSearch-board">
    ...
    %>
		<tr>
			...
			<td><a href="javascript:void(0)" onClick="window.opener.document.querySelector('.StorageCode').value='<%=rs.getString("STORAGR_ID")%>'; window.opener.document.querySelector('.StorageDes').value='<%=rs.getString("STORAGR_NAME")%>'; window.close();"><%=rs.getString("STORAGR_NAME") %></a></td>
		</tr>    
    <%    		
        	}while(rs.next());
        }
